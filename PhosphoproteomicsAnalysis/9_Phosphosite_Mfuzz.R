...
rawdata <- read.csv('../data/Raw_data/Phosphosite_Diffanalysis(p.adjust0.05_fc2).csv',
                    check.names = F, stringsAsFactors = F)

rownames(rawdata) <- rawdata$PhosphositeID
...
pdf('../data/Phosphosite_Mfuzz/Phosphosite_Mfuzz.pdf', 
    height = 8, width = 12)
mfuzz.plot(eset, mfuzz_cluster,mfrow=c(2,3),
           new.window=FALSE,min.mem=0.35,
           time.labels = unique(an_col$Type))
...
cluster_df <- data.frame(PhosphositeID = names(mfuzz_cluster$cluster),
                         Mfuzz_cluster = paste0('Cluster', mfuzz_cluster$cluster),
                         stringsAsFactors = FALSE)

data <- rawdata %>%
  left_join(cluster_df, by = "PhosphositeID") %>%
...
write.csv(data,file = '../data/Phosphosite_data/Phosphosite_Diffanalysis(p.adjust0.05_fc2).csv',
          row.names = F)
...
# -------- 7. 输出 PDF --------
pdf('../data/Phosphosite_Mfuzz/Phosphosite_Mfuzz_heatmap.pdf', width = 8, height = 8)
draw(hp)
dev.off()
